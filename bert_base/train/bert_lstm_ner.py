...
class NerProcessor(DataProcessor):
    ...
    # 返回一个类装载的 guid，text，lable >>> [<class 'bert_base.train.models.InputExample'>, ...]
    def get_train_examples(self, data_dir):
    ...
    def _create_example(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            # (train or text 等 ，  标注中每个句子的下标)
            guid = "%s-%s" % (set_type, i)
            # 对数据集进行编码，防止出现二进制'字'或者'标注'
            text = tokenization.convert_to_unicode(line[1])
            label = tokenization.convert_to_unicode(line[0])
            # 使用类装载 guid，text，lable
            examples.append(InputExample(guid=guid, text=text, label=label))
        # 返回一个类装载的 guid，text，lable >>> [<class 'bert_base.train.models.InputExample'>, ...]
        return examples

    # 加载bio数据集，返回[['我 爱 北 京 天 安 门', 'O O B-LOC I-LOC B-LOC I-LOC I-LOC'], ...]格式
    def _read_data(self, input_file):
        ...
        with codecs.open(input_file, 'r', encoding='utf-8') as f:
            ...
            for line in f:
                ...
                if contends.startswith("-DOCSTART-"):
                    continue
            # lines = [['我 爱 北 京 天 安 门', 'O O B-LOC I-LOC B-LOC I-LOC I-LOC'], ...]
            return lines
...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer, output_dir, mode):
    ...
    # O OR CLS 没有任何影响，不过我觉得O 会减少标签个数,不过句首和句尾使用不同的标志来标注，使用CLS 也没毛病
...
def train(args):
    ...
    # 下游任务要求句子的最大长度 不能 大于bert模型要求的句子长度
    ...
    # 构造基本的框架，包含提取bert的词文件vocab.txt,构造{字:index}, {index: 字} 和input的全部小写， 最大词块长度为100
    ...
    if args.do_train and args.do_eval:
        # 加载train数据 >>> # 返回一个类装载的 guid=train，text，lable >>> [<class 'bert_base.train.models.InputExample'>, ...]
        ...
        # 下载dev数据集 >>> # 返回一个类装载的 guid=dev，text，lable >>> [<class 'bert_base.train.models.InputExample'>, ...]
        eval_examples = processor.get_dev_examples(args.data_dir)
        # 打印验证集数据信息
        logger.info("***** Running evaluation *****")
        logger.info("   Num examples = %d", len(eval_examples))
        logger.info("b  Batch size = %d", args.batch_size)
    ...
    model_fn = model_fn_builder(
        bert_config=bert_config, # 加载好的 bert的config.json文件
